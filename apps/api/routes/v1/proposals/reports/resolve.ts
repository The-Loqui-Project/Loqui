...
export default {
  type: "POST",
  route: "/proposals/reports/:id/resolve",
  schema: {
    description: "Resolve a proposal report (for moderator+ roles)",
  ...
  func: async (request, response) => {
    ...
    try {
      ...
      const reportData = await db.query.proposalReport.findFirst({
      ...
      const [updatedReport] = await db
        .update(proposalReport)
        .set(updateData)
        .where(eq(proposalReport.id, reportId))
        .returning({
          id: proposalReport.id,
          status: proposalReport.status,
          resolvedAt: proposalReport.resolvedAt,
          resolvedById: proposalReport.resolvedById,
          resolutionNote: proposalReport.resolutionNote,
        });

...
export default {
  type: "POST",
  route: "/projects/reports/:id/resolve",
  schema: {
    description: "Resolve a project report (for moderator+ roles)",
  ...
  func: async (request, response) => {
    ...
    try {
      ...
      const reportData = await db.query.projectReports.findFirst({
      ...
      const [updatedReport] = await db
        ...
        .returning({
          ...
          resolvedAt: projectReports.resolvedAt,
          resolvedById: projectReports.resolvedById,
          resolutionNote: projectReports.resolutionNote,
        });

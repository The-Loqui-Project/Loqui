...
import db from "../../db";
import { item, language, proposal, project, translation, translationPack, versionTranslationPackStatus, versionToItem } from "../../db/schema/schema";
...
export async function processTranslationPacks() {
  ...
  taskManager.executeTask(taskId, async (updateProgress) => {
    ...
    const versionsByProject: Record<string, {
      projectId: string,
      versions: {
        versionId: string,
        languageCode: string
    ...
    for (let i = 0; i < projectIds.length; i++) {
      ...
      try {
        ...
        const result = await processProjectTranslationPacks(projectData.projectId, projectData.versions);
        results.push(result);
      } catch (error) {
        console.error(`Error processing translation packs for project ${projectId}:`, error);
        results.push({
          projectId,
          success: false,
          error: String(error)
      ...
      processedCount++;
      const progress = Math.floor((processedCount / projectIds.length) * 90) + 10;
...
async function processProjectTranslationPacks(
  ...
  const versionsByTranslations: Record<string, {
    versionIds: string[],
    languageCodes: string[],
  ...
  for (const { versionId, languageCode } of versionLangs) {
    try {
      ...
      const translatedCountQuery = await db.select({
        count: count()
      }).from(translation)
      .where(and(
      ...
      const meetsThreshold = translatedCount >= 45 || translatedPercentage >= 10;
      ...
      const translationFiles = await generateTranslationFilesForVersion(versionId);
      ...
      if (!versionsByTranslations[translationsHash].versionIds.includes(versionId)) {
        versionsByTranslations[translationsHash].versionIds.push(versionId);
      }
      
      if (!versionsByTranslations[translationsHash].languageCodes.includes(languageCode)) {
        versionsByTranslations[translationsHash].languageCodes.push(languageCode);
      }
      
      // Mark as released (not dirty anymore)
      await db.update(versionTranslationPackStatus)
      ...
      if (!packProject) {
        ...
        const [newPack] = await db.insert(translationPack)
          .values({
            projectId,
      ...
      console.error(`Error processing translation pack for version ${versionId}, language ${languageCode}:`, error);
      results.push({
        versionId,
        languageCode,
        meetsThreshold: false,
  ...
  for (const [translationsHash, data] of Object.entries(versionsByTranslations)) {
    if (!data.translationFiles || Object.keys(data.translationFiles).length === 0) {
      continue;
    }
    ...
    try {
      ...
      if (packPath) {
        console.log(`Generated resource pack for project ${projectId} with ${data.versionIds.length} version(s) at ${packPath}`);
        ...
        if (packProject) {
          await db.update(translationPack)
      ...
      console.error(`Error generating resource pack for project ${projectId}:`, error);
      results.push({
        versionIds: data.versionIds,
        languageCodes: data.languageCodes,
        generatedPack: false,
 ...
 */
export function hashTranslations(translationFiles: Record<string, Record<string, string>>): string {
...
export async function generateResourcePack(
  ...
  try {
    ...
    try {
      ...
      const existingPack = files.find(file => file.includes(`${projectInfo.slug || projectId}_${translationsHash}_`));
      
      if (existingPack) {
        console.log(`Using existing pack with same content hash: ${existingPack}`);
 ...
 */
export async function generateTranslationFilesForVersion(versionId: string): Promise<Record<string, Record<string, string>>> {
...
export function createModrinthResourcePack(projectId: string): string | PromiseLike<string | null> | null {
    return null;
}